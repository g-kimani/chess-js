  ...
  [
    ...
    tests.makeTest(
      ...
      () => {
        ...
        if (chess.enPassant !== null) {
          throw new Error(
            "Expected enPassant to be null got " + chess.enPassant
          );
        }
        return true;
      }
    ...
    () =>
      tests.testGroup("Chess - getSquare", [
        ...
        tests.makeTest(
          "getSquare returns null for an empty square",
          null,
          () => {
            const chess = new Chess();
            chess.start();
      ...
      tests.testGroup(
        ...
        [
          tests.makeTest("moves a piece with no validation", true, () => {
            ...
            const from = [0, 0];
            const to = [4, 4];
            const move = chess.move(from, to, false);
            return (
          ...
          tests.makeTest("moves a piece with validation", true, () => {
            ...
            const from = [6, 0];
            const to = [5, 0];
            const move = chess.move(from, to, true);
            return (
              chess.board[from[0]][from[1]] === null &&
          ...
          tests.makeTest("rejects an invalid move", false, () => {
            ...
            const from = [0, 0];
            const to = [4, 4];
            return chess.move(from, to, true);
          }),
          tests.makeTest(
          ...
          tests.makeTest(
            "sets an enpassant square after pawn move",
            true,
            () => {
              const chess = new Chess();
              chess.start();
          ...
          tests.makeTest(
            "increments fullMoveNumber after black move",
            3,
            () => {
              const chess = new Chess();
              chess.start();
    ...
    () =>
      tests.testGroup("insufficient material", [
        tests.makeTest(
